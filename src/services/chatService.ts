...
const updateConversationLastMessage = (conversationId: string) => {
    ...
    const lastMessage = messages[0]; // Messages are sorted newest to oldest? No, implementation above sorts oldest to newest.
    // Wait, getMessagesForConversation sorts a.timestamp - b.timestamp (oldest first).
    // So last message is at the end.
    // Let's re-sort here descending to get the newest easily.
    const messagesDesc = getMessages()
        .filter(m => m.conversationId === conversationId && m.isSent)
        .sort((a, b) => b.timestamp - a.timestamp);

    const newestMessage = messagesDesc[0];
    if (newestMessage) {
        let lastMsgText = newestMessage.content;
        if (newestMessage.isDeleted) {
            lastMsgText = 'Message deleted';
        } else if (newestMessage.isEdited) {
            lastMsgText = `(Edited) ${newestMessage.content}`;
        }
        conversation.lastMessage = lastMsgText;
        conversation.lastMessageSenderId = newestMessage.senderId;
        conversation.lastMessageTimestamp = newestMessage.timestamp;
    } else {
