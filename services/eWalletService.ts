...
import { EWallet, EWalletTransaction, SchoolFee, ParentalControlSettings, TransactionType, TopUpMethod, WithdrawalMethod, PinResetRequest, User, AdminUser, SchoolFeeItem, MarketplaceListing, Receipt, SchoolFeePaymentRecord, StagedSchoolFee } from '../types';
...
export const processBulkDisbursement = (payer: { id: string, name: string }, userIds: string[], amount: number, description: string): void => {
    ...
    const debitTxn = createTransaction({
        ...
        description: `Bulk disbursement: ${description} to ${userIds.length} users`,
        status: 'completed',
    });
...
export const releaseEscrowPayment = (receipt: Receipt, verifiedSellerId: string): void => {
    const currentStatus = receipt.statusHistory[receipt.statusHistory.length - 1]?.status;
    if (currentStatus !== 'Delivered') {
        throw new Error("Payment can only be released for delivered items.");
    }
    
    const seller = getUsers().find(u => u.studentId === receipt.sellerId);
    ...
    if (!extractedId || extractedId.toLowerCase() !== receipt.sellerId.toLowerCase()) {
        throw new Error("Seller ID does not match the recipient of this purchase.");
    }

    const buyer = getUsers().find(u => u.studentId === receipt.buyerId);
    if (!buyer) {
        throw new Error("Could not identify the buyer for this transaction.");
    }

    // Transfer from escrow to seller, making it look like it came from the buyer for the seller's log
    makePayment(
        MARKETPLACE_ESCROW_USER_ID,
        receipt.sellerId,
        receipt.amount,
        `Payment for Order ID: ${receipt.orderId.slice(-8)}`,
        'payment',
        { overrideSenderName: buyer.name }
    );

    // Create a new, zero-amount transaction for the buyer as a confirmation log.
    createTransaction({
        walletUserId: receipt.buyerId,
        type: 'payment',
        amount: 0, // This is just a log entry, no balance change.
        description: `Payment released for Order ID: ${receipt.orderId.slice(-8)} to ${seller.name}`,
        status: 'completed',
        recipient: seller.name,
        method: 'e-wallet',
    });

    // Update status to Completed for both receipts (buyer and seller)
    receiptService.updateReceiptStatus(receipt.id, 'Completed');
};
