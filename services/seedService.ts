...
export const seedInitialData = () => {
    ...
    try {
        const defaultModules = [
            ...
            { name: EXPLORATION_MODULE_NAME, description: 'Provides interactive self-service content via the Kiosk system.', isAssignable: true },
    ...
    try {
        ...
        if (!homePageModule || !smartAdmissionModule || !messageModule || !eWalletModule || !onlineModule || !smartStudentIdModule || !eCanteenModule || !ncheModule || !explorationModule || !studentTransferModule || !newsFeedModule || !eVoteModule || !myInstituteModule) {
        ...
        const school1Modules = [smartAdmissionModule.id, messageModule.id, eWalletModule.id, onlineModule.id, smartStudentIdModule.id, eCanteenModule.id, ncheModule.id, explorationModule.id, studentTransferModule.id, newsFeedModule.id, eVoteModule.id, myInstituteModule.id];
