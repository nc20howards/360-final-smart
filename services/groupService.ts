import { Group, GroupPost, Story, BroadcastChannel, BroadcastMessage, ChatMessage, PostComment, Event, Place, ChatAttachment, NegotiationMessage } from '../types';
...
export const getPostsForGroup = (groupId: string): GroupPost[] => {
    return getPosts()
        ...
        .sort((a: GroupPost, b: GroupPost) => a.timestamp - b.timestamp); // Sort as a chat (oldest first)
};
...
export const getStoriesGroupedByUser = (): Record<string, Story[]> => {
    ...
    for (const story of activeStories) {
        ...
        // FIX: Explicitly type sort parameters to prevent type inference issues.
        grouped[story.userId].sort((a: Story, b: Story) => a.timestamp - b.timestamp);
    }
    return grouped;
};
...
export const getMessagesForChannel = (channelId: string): BroadcastMessage[] => {
    return getBroadcastMessages()
        ...
        // FIX: Explicitly type sort parameters to prevent type inference issues.
        .sort((a: BroadcastMessage, b: BroadcastMessage) => a.timestamp - b.timestamp);
};
...
export const getAllEventsForSchool = (schoolId: string): Event[] => {
    // FIX: Explicitly type sort parameters to prevent type inference issues.
    return getEvents().filter(e => e.schoolId === schoolId).sort((a: Event, b: Event) => b.createdAt - a.createdAt);
};
