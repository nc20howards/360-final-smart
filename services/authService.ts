...
export const loginUser = (studentId: string, password?: string): User | null => {
    ...
    if (studentId.toLowerCase() === SUPERADMIN_ID && password === SUPERADMIN_PASS) {
        ...
        logAction(superadminUser.studentId, superadminUser.name, 'LOGIN', { role: 'superadmin' });
    ...
    if (user && user.accountStatus === 'disabled') {
        ...
        logAction(studentId, user.name, 'LOGIN_FAILED', { reason: 'Account Disabled' });
    ...
    if (user && isPasswordMatch) {
        ...
        logAction(user.studentId, user.name, 'LOGIN', { role: user.role, schoolId: user.schoolId });
        return user;
    }
    
    if (user && !isPasswordMatch) {
         logAction(studentId, user.name, 'LOGIN_FAILED', { reason: 'Incorrect Password' });
    }
...
export const loginAdminUser = (email: string, password: string): AdminUser | null => {
    ...
    if (user) {
        logAction(user.id, user.name, 'ADMIN_LOGIN_FAILED', { role: user.role });
    }
...
export const logout = (): void => {
    ...
    if (user) {
         const id = 'studentId' in user ? user.studentId : user.id;
         logAction(id, user.name, 'LOGOUT', { role: user.role });
    }
