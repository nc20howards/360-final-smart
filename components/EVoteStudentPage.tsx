...
const EVoteStudentPage: React.FC<EVoteStudentPageProps> = ({ user, school }) => {
    ...
    const handleSelectContestant = (categoryId: string, contestantId: string) => {
        if (hasVoted) return; // Prevent changing selections after voting
         ...
         return (
            <div className="animate-slide-in-left-fade">
                ...
                <div className="mt-8 text-center">
                    <button onClick={handleSaveChoices} disabled={Object.keys(selections).length !== categories.length || hasVoted} className="px-8 py-3 bg-cyan-600 hover:bg-cyan-700 rounded-lg font-bold text-lg disabled:bg-gray-600 disabled:cursor-not-allowed">
