...
import { School, AdminUser, StudentTransferProposal, TransferNegotiation, CompletedAdmission, SchoolClass, EWallet, StudentTransferRequest } from '../types';
...
const StudentTransferMarketplace: React.FC<StudentTransferMarketplaceProps> = ({ school, user, classes }) => {
    ...
    const refreshData = useCallback(() => {
        ...
        const newMarketItems = marketProposalsData.filter(p => p.timestamp > lastMarketVisit).length +
                               allRequests.filter(r => r.timestamp > lastMarketVisit && r.requestingSchoolId !== school.id).length; 
    ...
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        setProposalForm(prev => {
            ...
            if (name === 'levelCategory') {
                newState.className = '';
                newState.stream = ''; // Reset stream when level category changes
            }
            if (name === 'className') {
                newState.stream = ''; // Reset stream when class changes
            }
            return newState;
        });
        setFormError('');
    ...
    const handleConfirmFulfillRequest = () => {
        ...
        try {
            const privateProposal = schoolService.createProposal({
                ...
                stream: requestToFulfill.stream, // Include stream from request
    ...
    const handleCreateProposal = () => {
        const { pricePerStudent, numberOfStudents, grade, className, gender, stream } = proposalForm;
        ...
        if (selectedClass && selectedClass.streams.length > 0 && !stream) {
            setFormError("Stream is required for the selected class.");
            return;
        }

        // --- Robust Validation Logic ---
        const stagedForClass = stagedStudents.filter(s => s.targetClass === className && (stream ? s.stream === stream : true)); // Filter by stream
        const staged_male_count = stagedForClass.filter(s => s.gender === 'Male').length;
        const staged_female_count = stagedForClass.filter(s => s.gender === 'Female').length;
        
        const committed_male = myProposals.filter(p => p.status === 'open' && p.className === className && (stream ? p.stream === stream : true) && p.gender === 'Male').reduce((sum, p) => sum + p.numberOfStudents, 0);
        const committed_female = myProposals.filter(p => p.status === 'open' && p.className === className && (stream ? p.stream === stream : true) && p.gender === 'Female').reduce((sum, p) => sum + p.numberOfStudents, 0);
        const committed_mixed = myProposals.filter(p => p.status === 'open' && p.className === className && (stream ? p.stream === stream : true) && p.gender === 'Mixed').reduce((sum, p) => sum + p.numberOfStudents, 0);
        ...
        if (gender === 'Mixed') {
            ...
            if (numStudentsToPropose > available_pool - committed_mixed) {
                 setFormError(`Cannot propose ${numStudentsToPropose} students. Only ${available_pool - committed_mixed} are available in the unallocated pool for this class/stream.`);
                 return;
            }
        }
        // --- End Validation ---
    ...
    const handleConfirmPayment = () => {
        ...
        try {
            ...
            const proposal = schoolService.getProposals().find(p => p.id === selectedNegotiation.proposalId); // FIX: Use schoolService.getProposals()
                        ...
                        <div key={p.id} className="bg-gray-800 p-4 rounded-lg border-l-4 border-cyan-500">
                             <div className="flex justify-between items-start">
                                <div className="flex-grow">
                                    ...
                                    <p className="text-sm text-gray-300">Offering <span className="font-semibold">{p.numberOfStudents} {p.gender}</span> students in <span className="font-semibold">{p.className} {p.stream ? `(${p.stream})` : ''}</span></p>
                                ...
                                {myProposals.filter(p => p.status === 'open').map(p => {
                                   ...
                                   return (
                                       <div key={p.id} className="bg-gray-800 p-4 rounded-lg">
                                           ...
                                           <p className="text-sm text-gray-300">Class: {p.className} {p.stream ? `(${p.stream})` : ''} | Grade: {p.grade}</p>
    ...
    const renderNegotiations = () => {
        ...
        const otherSchoolAvatar = (negotiation: TransferNegotiation) => {
            ...
            const otherSchool = schoolService.getAllSchools().find(s => s.id === otherSchoolId);
            if (otherSchool) {
                const homeContent = getHomePageContent(otherSchool.id); // FIX: Correctly imported from homePageService
                ...
                <>
                    ...
                    <footer className="p-3 border-t border-gray-700">
                        <div className="flex gap-2">
                            <input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Type your message..." className="w-full bg-gray-700 p-2 rounded-md" />
                    ...
                    <div className="bg-gray-800 rounded-lg w-full max-w-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 space-y-4 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="text-xs text-gray-400">Stream</label>
                                    <select name="stream" value={proposalForm.stream} onChange={handleFormChange} disabled={!proposalForm.className || classes.find(c => c.name === proposalForm.className)?.streams.length === 0} className="w-full p-2 bg-gray-700 rounded mt-1 disabled:bg-gray-600">
                                        <option value="">Select Stream</option>
                                        {classes.find(c => c.name === proposalForm.className)?.streams.map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                </div>
                                <div>
                    ...
                    <div className="bg-gray-800 rounded-lg p-6 w-full max-w-lg space-y-4">
                        ...
                        <div className="bg-gray-700/50 p-4 rounded-lg border border-yellow-500/50">
                            ...
                            <ul className="text-sm mt-2 space-y-1">
                                ...
                                <li><span className="text-gray-400">For Class:</span> {requestToFulfill.className} ({requestToFulfill.grade})</li>
                ...
                return (
                    <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4 animate-fade-in-up">
                        <div className="bg-gray-800 rounded-lg p-6 w-full max-w-lg space-y-4 border border-gray-600 shadow-xl max-h-[90vh] overflow-y-auto">
                            ...
                            {/* NEW: Conditional display for requesting school (buyer) showing original request vs fulfillment offer */}
                            {originalRequest && isBuyer && (
                                <div className="bg-gray-700/50 p-4 rounded-lg border border-yellow-500/50">
                                    ...
                                    <ul className="text-sm mt-2 space-y-1">
                                        ...
                                        <li><span className="text-gray-400">For Class:</span> {originalRequest.className} ({originalRequest.grade})</li>
                            ...
                            {/* Standard proposal details (not a fulfillment offer or for the seller) */}
                            {(!originalRequest || !isBuyer) && ( 
                                <div className="bg-gray-700/50 p-4 rounded-lg space-y-2 text-sm">
                                    ...
                                    <div className="flex justify-between"><span className="text-gray-400">Class:</span><span className="font-semibold">{proposal.className} {proposal.stream ? `(${proposal.stream})` : ''}</span></div>
                            ...
                            <div className="border-t border-gray-700 pt-4">
                                ...
                                {isBuyer ? (
                                    <>
                                        {isPaymentMade ? (
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <input type="number" placeholder="Set Your Offer (UGX per student)" value={buyerOffer} onChange={e => setBuyerOffer(e.target.value)} disabled={allControlsDisabled} className="w-full p-2 bg-gray-700 rounded" />
                                                    <button onClick={() => handleUpdateTerms({ buyerOffer: Number(buyerOffer) })} disabled={allControlsDisabled} className="px-4 py-2 bg-cyan-600 rounded font-semibold whitespace-nowrap">Submit Offer</button>
                                                </div>
                                            </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {!isPaymentMade && (
                                            <div className="border-t border-gray-600 pt-3 mt-3 flex flex-wrap gap-2">
                                                <button onClick={() => {const price = prompt("Set final non-negotiable price per student:"); if (price && !isNaN(Number(price))) {handleUpdateTerms({ finalPrice: Number(price), status: 'accepted' });}}} disabled={allControlsDisabled} className="px-3 py-1 bg-green-700 rounded text-sm">Set Final Price</button>
                                                <button onClick={handleRejectAllNegotiations} disabled={allControlsDisabled} className="px-3 py-1 bg-red-700 rounded text-sm">Reject All Negotiations</button>
                                            </div>
                                        )}
                                    </div>
                                )}
