...
const OrderTrackingModal: React.FC<OrderTrackingModalProps> = ({ receipt, onClose }) => {
    // Define the logical flow of an order
    const steps = [
    ...
    const isCompleted = currentStatus === 'Completed'; // Treated as post-delivery

    // Determine the active step index based on current status
    // If completed, consider it fully delivered.
    ...
    if (isCompleted) {
        ...
        activeStepIndex = steps.findIndex(s => s.status === currentStatus);
    }
    
    // Fallback if status isn't found (e.g. legacy data)
    if (activeStepIndex === -1 && !isCancelled) activeStepIndex = 0;

    return (
        <div className="fixed inset-0 bg-black/80 flex justify-center items-center z-[160] p-4 animate-fade-in-up">
            <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md shadow-2xl border border-gray-700">
                <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
                    <div>
                         ...
                         <p className="text-xs text-gray-400">Order #{receipt.orderId.slice(-6)}</p>
                    </div>
                ...
                {isCancelled ? (
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                        ...
                        <h4 className="text-xl font-bold text-red-400">Order Cancelled</h4>
                        <p className="text-sm text-gray-400 mt-2">This order has been cancelled by the seller or system.</p>
                    </div>
                ) : (
                    <div className="space-y-0 relative">
                        {/* Connecting Line */}
                        <div className="absolute left-[19px] top-4 bottom-8 w-0.5 bg-gray-700 z-0"></div>

                        {steps.map((step, index) => {
                            const isCompleted = index <= activeStepIndex;
                            const isCurrent = index === activeStepIndex;
                            
                            // Find timestamp for this specific status in history if it exists
                            const statusLog = receipt.statusHistory.find(h => h.status === step.status);
                            const timestamp = statusLog ? new Date(statusLog.timestamp).toLocaleString() : null;

                            return (
                                <div key={step.status} className="flex gap-4 relative z-10 pb-8 last:pb-0">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 flex-shrink-0 transition-colors duration-300 ${isCompleted ? 'bg-green-600 border-green-600 text-white' : 'bg-gray-800 border-gray-600 text-gray-600'}`}>
                                        {isCompleted ? (
                                    ...
                                    <div className="pt-1">
                                        <h4 className={`font-bold ${isCompleted ? 'text-white' : 'text-gray-500'} ${isCurrent ? 'text-cyan-400' : ''}`}>
                                            {step.label}
                                        </h4>
                                        <p className="text-xs text-gray-400 mt-0.5">
                                            {isCompleted && timestamp ? timestamp : index === 0 ? 'Waiting...' : ''}
                                        </p>
                                    </div>
                                </div>
                            );
                ...
                <div className="mt-8 pt-4 border-t border-gray-700">
                    <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="font-bold text-white">UGX {receipt.amount.toLocaleString()}</span>
                    </div>
                     <div className="flex justify-between text-sm">
                        <span className="text-gray-400">Seller</span>
                        <span className="font-bold text-cyan-400">{receipt.partyName}</span>
                    </div>
                </div>
            </div>
        </div>
...
const ReleasePaymentModal: React.FC<ReleasePaymentModalProps> = ({ receipt, onClose, onSuccess }) => {
    ...
    const handleRelease = async () => {
        ...
        try {
            ...
            // Keep isReleasing true to prevent double clicks while success message/action processes
            onSuccess("Payments successfully made");
        } catch (err) {
            setError((err as Error).message);
            setIsReleasing(false); // Only re-enable on error
        }
    };
...
const MarketplaceView: React.FC<{
    ...
    onOrderStaged?: (order: StagedMarketplaceOrder) => void;
}> = ({ user, setViewingListing, onAddToCart, refreshAllData, initialTab, onClearCart, cart, onNavigateToWallet, onOrderStaged }) => {
    ...
    const [trackingReceipt, setTrackingReceipt] = useState<Receipt | null>(null); // NEW: Track which receipt is being tracked
    ...
    const handleOrderStatusUpdate = (receiptId: string, newStatus: ReceiptStatus) => {
        try {
            ...
            fetchReceipts(); // Refresh local receipts list
        ...
        <div className="space-y-4">
            ...
            {trackingReceipt && (
                <OrderTrackingModal 
                    ...
                    onClose={() => setTrackingReceipt(null)} 
                />
            )}
                            ...
                            return (
                                <div key={receipt.id} className="bg-gray-800 p-4 rounded-lg shadow-md border-l-4 border-cyan-500">
                                    ...
                                    <div className="mt-4 flex flex-wrap justify-end gap-2">
                                        <button 
                                            onClick={() => setTrackingReceipt(receipt)}
                                            className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-md font-semibold text-sm transition-colors border border-gray-600"
                                        >
                                            Track Order
                                        </button>
                    ...
                    {receiptToRelease && (
                        <ReleasePaymentModal 
                            ...
                            onSuccess={(msg) => {
                                ...
                                fetchReceipts(); // Refresh local list immediately
                                refreshAllData(); 
                            }}
                        />
                    )}
...
export const OnlineFeedPage: React.FC<OnlineFeedPageProps> = ({ user, onLogout, onBackToDashboard, onStartMessage, onNavigateToWallet, onOrderStaged, stagedOrder, onStagedOrderConsumed }) => {
    ...
    const handleConfirmMarketplacePayment = (pin: string) => {
        ...
        try {
            ...
            // FIX: Call the prop to clear parent state. This hides the "Finish Shopping" banner on E-Wallet.
