...
const EWalletPage: React.FC<EWalletPageProps> = ({ user, stagedMarketplaceOrder, onFinishShopping, stagedCanteenOrder, onProceedWithCanteenOrder }) => {
    const [view, setView] = useState<'dashboard' | 'transactions' | 'receipts'>('dashboard');
    ...
    const refreshData = useCallback(() => {
        ...
        if (isSuperadmin) {
            ...
            // PRIVACY FIX: Headteacher dashboard should only show their own transactions, not school-wide ones.
            setTransactions(eWalletService.getTransactionsForUser(currentUserId));
    ...
    const handleBulkDisbursement = () => {
        ...
        try {
            eWalletService.processBulkDisbursement({ id: currentUserId, name: user.name }, selectedUserIds, disbursementAmount, disbursementDescription);
            showFeedback(`Disbursed ${formatCurrency(disbursementAmount)} to ${selectedUserIds.length} users. Funds have been deducted from your account.`);
            refreshData();
            setModal(null);
    ...
    return (
        <div className="h-full">
            ...
            {view === 'dashboard' ? (isHeadteacher ? renderHeadteacherDashboard() : renderPersonalWalletDashboard()) : 
             view === 'receipts' ? renderReceiptsView() : renderTransactionsView()}
        </div>
    );
};
