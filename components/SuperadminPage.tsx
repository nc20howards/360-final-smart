...
import { AdminUser, School, Module, User as SchoolUser, AuditLogEntry, UnebStats, IpWhitelistSettings, HigherEducationInstitution, Program, UnebLogoSettings } from '../types';
...
export const SuperadminPage: React.FC<SuperadminPageProps> = ({ user, onLogout }) => {
    ...
    const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onload = () => {
                setUnebLogoSettings(prev => ({ ...prev, url: reader.result as string }));
            };
                ...
                return (
                    <div className="bg-gray-800 rounded-lg shadow-xl p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-4 border-t md:border-t-0 md:border-l border-gray-700 pt-6 md:pt-0 md:pl-8">
                                <h4 className="text-lg font-semibold">Logo Customization</h4>
                                <div className="flex items-center gap-4">
                                    <img src={unebLogoSettings.url} alt="UNEB Logo Preview" className="w-24 h-24 rounded-full bg-white p-2 object-contain" />
                                    <div>
                                        <label className="block text-sm font-medium text-gray-300 mb-1">Upload New Logo</label>
                                        <input type="file" onChange={handleLogoUpload} accept="image/*" className="text-sm text-gray-400 file:mr-4 file:py-1 file:px-2 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-gray-600 file:text-white hover:file:bg-gray-500"/>
                                    </div>
                                </div>
                                <div>
                                    <label htmlFor="logo-size" className="block text-sm font-medium text-gray-300 mb-1">Logo Size: {unebLogoSettings.size}px</label>
                                    <input
                                        id="logo-size"
                                        type="range"
                                        min="24"
                                        max="96"
                                        value={unebLogoSettings.size}
                                        onChange={e => setUnebLogoSettings(prev => ({ ...prev, size: parseInt(e.target.value, 10) }))}
                        ...
                        <div className="mt-6 text-right">
                            <button onClick={handleUnebSettingsSave} className="px-6 py-2 bg-cyan-600 hover:bg-cyan-700 rounded-md font-semibold">Save All UNEB Settings</button>
                        </div>
                    </div>
                );
    ...
    const navLinks = [
        ...
        // Add new link for My Institute
        { name: 'My Institute', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, view: 'my_institute' },
    ];
    ...
    return (
        <div className="flex h-screen bg-gray-900 text-white font-sans">
            ...
            {/* --- Confirmation Modal --- */}
            {modal?.type === 'deleteSchool' && (
                <ConfirmationModal 
                    isOpen={true} 
                    title="Delete School" 
                    message={`Are you sure you want to delete ${modal.data.schoolName}? This action cannot be undone.`} 
                    onConfirm={() => confirmSchoolDelete(modal.data.schoolId)} 
            ...
            {modal?.type === 'deleteUser' && (
                <ConfirmationModal 
                    isOpen={true} 
                    title="Delete User" 
                    message={`Are you sure you want to delete ${modal.data.userName}? This action cannot be undone.`} 
                    onConfirm={() => confirmUserDelete(modal.data.userId)} 
                    onCancel={() => setModal(null)} 
                    confirmButtonVariant="danger"
                />
            )}
            {modal?.type === 'deleteModule' && (
                 <ConfirmationModal 
                    isOpen={true} 
                    title="Delete Module" 
                    message={`Are you sure you want to delete the module "${modal.data.moduleName}"? This will remove it from all assigned schools.`} 
                    onConfirm={() => confirmModuleDelete(modal.data.moduleId)} 
                    onCancel={() => setModal(null)} 
                    confirmButtonVariant="danger"
                />
            )}
            {modal?.type === 'resetPassword' && (
                 <ConfirmationModal 
                    ...
                    message={`Are you sure you want to reset the password for ${modal.data.name}?`} 
