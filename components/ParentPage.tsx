...
export const ParentPage: React.FC<ParentPageProps> = ({ user, onLogout }) => {
    const [currentView, setCurrentView] = useState('dashboard');
    const [school, setSchool] = useState<School | null>(null);
    const [schoolName, setSchoolName] = useState('Parent Portal');
        ...
        if (currentView === 'dashboard') {
            return (
                <div className="flex flex-col items-center justify-center h-full text-center space-y-6 animate-fade-in-up">
                    ...
                    <h1 className="text-3xl font-bold text-cyan-400">Parent Portal</h1>
                    <p className="text-lg text-gray-300">Welcome, {currentUser.name}.</p>
                    <p className="text-gray-400 max-w-md">Use the sidebar to access school services assigned to you, such as Messages, News, or Payment portals.</p>
                </div>
            );
        }
                 ...
                 case SMART_STUDENT_ID_MODULE_NAME:
                    return (
                        <div className="flex flex-col items-center justify-center min-h-[60vh]">
                            <h2 className="text-2xl font-bold text-white mb-8">Parent ID Card</h2>
                             <div className="bg-gray-800 p-8 rounded-lg text-center">
                                <p className="text-gray-400">Digital ID for parents is coming soon.</p>
                             </div>
                        </div>
                    );
                default:
